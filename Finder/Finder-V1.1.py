...
def finder(env, stringToSearch, stringToExclude, useExcludeFlag, isOnlySystem):
    ...
    return (resultList, numOfFoundJobs)
    ...
    for subItem in objectName:
        ...
        if parameterType == "VARIABLE":
            ...
            if stringToSearch in lowerInitName:
                ...
                tempList.append(listRow)
        elif parameterType == "CONTROL":
            initName = subItem.attrib.get('NAME')
            initValue = subItem.attrib.get('VALUE')
            lowerInitName = initName.lower()
            if stringToSearch in lowerInitName:
                foundIndicator = True
                listRow = "---->> CONTROL RESOURCE = " + initName
                tempList.append(listRow)
        ...
        if parameterType == "AUTOEDIT2":
            ...
            for doStatement in subItem.iter():
                ...
                if doType == "DOSHOUT":
                    ...
                    msgText = doStatement.attrib.get('MESSAGE')
                    lowerMsgText = msgText.lower()
                    if (stringToSearch in lowerDestName and stringToExclude not in lowerDestName) or (
                            stringToSearch in lowerMsgText and stringToExclude not in lowerMsgText):
                        foundIndicator = True
                        listRow = "---->> On Code = " + onCode + " Stmt = " + onStmt + " | Shout Destination Name = " + destName + " | MSG = " + msgText
                        tempList.append(listRow)
                if doType == "DOCOND":
                ...
                if doType == "DOMAIL":
                    ...
                    if mailSubject != None:
                        ...
                        mailSubject = lowerMailSubject = "Empty Subject"
                    if (stringToSearch in lowerMailDest and stringToExclude not in lowerMailDest) or (
                            stringToSearch in lowerMailMsg and stringToExclude not in lowerMailMsg) or (
                            stringToSearch in lowerMailSubject
                    and stringToExclude not in lowerMailSubject):
                        foundIndicator = True
                        listRow = "---->> Mail Message:" + "\n-------->> TO : " + mailDestination + "\n-------->> Subject : " + mailSubject + "\n-------->> Message : " + mailMsg
                        tempList.append(listRow)
        elif parameterType == "SHOUT":
...
if __name__ == "__main__":
    yes = set(['YES', 'Y', 'YE'])
    prodEnv = set(['P', 'p', 'prod', 'Prod', 'PROD'])
    testEnv = set(['T', 't', 'test', 'Test', 'TEST'])
    numOfFoundJobs = 0
    envToSearchIn = ""
    stringToSearch = ""
    stringToExclude = ""
    ...
    while keepSearchingIndicator:
        ...
        envToSearchIn = ""
        while envToSearchIn == "":
            envToSearchIn = input("In Which Environment would you like to search? (Prod/Test) : ")
            if envToSearchIn in prodEnv:
                envToSearchIn = "Prod"
            elif envToSearchIn in testEnv:
                envToSearchIn = "Test"
            else:
                print("Wrong env selection")
                envToSearchIn = ""
        isOnlySystem = input("Search only in System Folders? (y/yes or any othter key continue) : ")
        if isOnlySystem in yes:
            isOnlySystem = True
        else:
            isOnlySystem = False
        ...
        if useExclude in yes:
            ...
            stringToSearch = stringToSearch.strip()
            stringToExclude = stringToExclude.strip()
            finderResults, numOfFoundJobs = finder(envToSearchIn, stringToSearch, stringToExclude, useExcludeFlag, isOnlySystem)
        else:
            useExcludeFlag = False
            stringToSearch = stringToSearch.strip()
            finderResults, numOfFoundJobs = finder(envToSearchIn ,stringToSearch, "None", useExcludeFlag, isOnlySystem)
        if isinstance(finderResults, list):
            for subList in finderResults:
                print("\n".join(subList))
        else:
            print(finderResults)
        print("\n**************************************\nTotal number of jobs found : ", numOfFoundJobs)
        keepSearching = input('\n\ndo you want to search another string ? (y/yes)')
